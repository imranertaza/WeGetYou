...
-->
         <main id="content">
            <section>
               <form method="POST" class="d-flex justify-content-left"  action="{{ url('/savefreeTimeoptions')}} ">
                  <article class="question has-pageAnimation type_multi_column_checkbox REMOVE_FREETEXT_ANSWERS_FROM_QUESTIONNAIRE_BY_KEX "
                     ...
                     <header>
                        <h1 class="row" role="heading" aria-level="1">
                           <p class="questionText">What do you like to do in your free time? </p>
                        </h1>
                     ...
                     <div class="row">
                        <ul class="column">
                           @csrf
							...
							@foreach($freetimeoptions as $freetimeoption)
							<li class="answerWrapper qa-answerWrapper">
								<input type="checkbox" id="freetimeoption{{ $freetimeoption['id'] }}" name="freetimeoption[]" value="{{ $freetimeoption['id'] }}" />
								<label for="freetimeoption{{ $freetimeoption['id'] }}">
								{{ $freetimeoption['free_time_options'] }} </label>
							</li>
							@endforeach
