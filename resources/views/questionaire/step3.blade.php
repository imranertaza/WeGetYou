...
-->
         <main id="content">
            <section>
               <form method="POST" class="d-flex justify-content-left"  action="{{ url('/savefoodpreference')}} ">
                    @csrf
                  <article class="question has-pageAnimation type_multi_column_checkbox REMOVE_FREETEXT_ANSWERS_FROM_QUESTIONNAIRE_BY_KEX "
                     ...
                     <header>
                        <h1 class="row" role="heading" aria-level="1">
                           <p class="questionText">What food do you prefer? </p>
                        </h1>
                        <div class="hintAndErrors is-static">
                           <p class="fillOutHint">Up to 5 answers possible.</p>
                     ...
                     <div class="row">
                        <ul class="column">
							...
							<li class="answerWrapper qa-answerWrapper">
								<input class="checkbox_check" type="checkbox" id="food_pref{{ $preference['id'] }}" name="food_pref[]" value="{{ $preference['id'] }}" />
								<label for="food_pref{{ $preference['id'] }}">
								{{ $preference['food_pref'] }} </label>
							</li>
							@endforeach
